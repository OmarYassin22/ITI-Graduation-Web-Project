...
export async function PUT(request) {
  try {
    const res = new URL(request.url);
    const id = res.pathname.substring(res.pathname.lastIndexOf("/") + 1);
    const { title, price, cImage, details, duration, instructor, buyers, track } = await request.json();
    const oldDoc = doc(db, "courses", id);
    updateDoc(oldDoc, { title, price, cImage, details, duration, instructor, buyers, track });
    return NextResponse.json({
